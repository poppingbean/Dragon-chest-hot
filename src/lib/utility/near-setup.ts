...
export const CONTRACT_ID = "egg.dragongames.near";
...
export const initNear = async (): Promise<{ selector: WalletSelector, modal: WalletSelectorModal }> => {
...
export const callFunction = async (functionName: string, args = {}, deposit = '1'): Promise<any> => {
  try {
    const account = selector.store.getState().accounts[0];
