<script lang="ts">
    ...
    import Popup from '$lib/component/chest/components/Popup.svelte';
    ...
    onMount(async () => {
        const { selector, modal } = await initNearChest();
        console.log(selector);
        try{
            wallet = await selector.wallet();
            console.log(wallet);
            
            const accounts = await wallet.getAccounts();; // Ensure accounts are fetched correctly
            if (accounts.length > 0) {
                walletAccount.set(true);
                accountId.set(accounts[0].accountId);
                signedAccountId = accounts[0].accountId;
                if (signedAccountId) {
    ...
    const checkRegisterdAccount = async () => {
        ...
        if(storageResult === null || storageResult === undefined){
            try{
                ...
                console.log(depositeStorageResult);
...
<style>
    ...
    @media (max-width: 344px) {
        ...
        .cloud {
            width: 200px;
            top: -360px; /* Điều chỉnh vị trí đám mây so với rồng */
            left: 70px;
        }
